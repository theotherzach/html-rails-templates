  ...
  class Engine < ::Rails::Engine
    ...
    config.before_configuration do |app|
      ...
      each do |ext|
        begin
          silence_warnings do
            config.html_js_templates.markups << ext if Tilt[ext]
          end
        rescue LoadError
    ...
    initializer 'angular-rails-templates', group: :all  do |app|
      if app.config.assets
        ...
        if app.config.html_js_templates.htmlcompressor
          require 'htmlcompressor/compressor'
          unless app.config.html_js_templates.htmlcompressor.is_a? Hash
            app.config.html_js_templates.htmlcompressor = {remove_intertag_spaces: true}
          end
        end

        # These engines render markup as HTML
        app.config.html_js_templates.markups.each do |ext|
      ...
      app.config.assets.version = [
        app.config.assets.version,
        'ART',
        Digest::MD5.hexdigest("#{VERSION}-#{app.config.html_js_templates}")
      ].join '-'
    end
    ...
    config.after_initialize do |app|
      ...
      if app.config.html_js_templates.ignore_prefix.is_a? String
        app.config.html_js_templates.ignore_prefix = Array(app.config.html_js_templates.ignore_prefix)
      end
    end
  end
end
